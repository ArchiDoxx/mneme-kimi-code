...
PYPI_URL = "https://pypi.org/pypi/mneme-kimi-code/json"
CACHE_FILE = Path.home() / ".kimi-code" / "mneme" / ".version_cache"
...
def fetch_latest_version() -> str | None:
    ...
    try:
        req = Request(PYPI_URL, headers={"User-Agent": f"mneme-kimi-code/{__version__}"})
...
def upgrade_package() -> bool:
    ...
    try:
        result = subprocess.run(
            [sys.executable, "-m", "pip", "install", "--upgrade", "mneme-kimi-code"],
            capture_output=True,
            text=True,
            check=False,
        )
...
|     or: pip install --upgrade mneme-kimi-code|
+-----------------------------------------+
""")
