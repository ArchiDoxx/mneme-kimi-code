...
| Raw `~/.kimi-code/credentials` | ❌ No | Only OAuth token is read locally for auth |

> **Important:** The OAuth token from `~/.kimi-code/credentials/kimi-code.json` is read locally and used only for Kimi API authentication. It is never included in the data sent for structuring or compression. When using Ollama or other local LLMs, no external network calls are made.
