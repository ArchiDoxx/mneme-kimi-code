...
2. Register via `mneme bootstrap` or manually in `~/.kimi-code/config.toml`:
...
# Inspect database
python -c "import sqlite3; conn=sqlite3.connect('~/.kimi-code/mneme/mneme.db'); [print(r) for r in conn.execute('SELECT * FROM observations LIMIT 10').fetchall()]"
# Or if sqlite3 CLI is installed: sqlite3 ~/.kimi-code/mneme/mneme.db "SELECT * FROM observations LIMIT 10;"
