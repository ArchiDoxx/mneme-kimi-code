...
```bash
kimi plugin install /path/to/mneme-kimi-code/plugin
```

> **Hinweis für Kimi Code CLI:** Der Befehl `kimi plugin install` wird von Kimi Code CLI **nicht** unterstützt. Für Kimi Code CLI wird die Integration stattdessen via `mneme bootstrap` eingerichtet, das Hooks in `~/.kimi-code/config.toml` und den MCP-Server in `~/.kimi-code/mcp.json` registriert. Siehe [INSTALL.md](INSTALL.md) und [README.md](../README.md).
