...
Default location: `~/.kimi-code/mneme/config.json`
...
```json
{
  ...
  "logging": {
    "level": "INFO",
    "file": "~/.kimi-code/mneme/mneme.log",
    "max_size_mb": 10,
    "backup_count": 5
  }
}
...
|-----|------|---------|-------------|
| `db.path` | string | `~/.kimi-code/mneme/mneme.db` | SQLite database file path |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `vector.path` | string | `~/.kimi-code/mneme/vectors` | Vector store directory (embeddings cache) |
...
| `logging.file` | string | `~/.kimi-code/mneme/mneme.log` | Log file path |
